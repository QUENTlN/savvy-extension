...
function validateConsolidationWithReShipping() {
  const consolidationMode = document.querySelector(
    'input[name="consolidationMode"]:checked'
  )?.value;
  const reShippingContainer = document.querySelector(
    '[data-fee-section="reShipping"] .calculation-rules-container'
  );
  const reShippingType = reShippingContainer?.querySelector(
    ".calculation-type-radio:checked"
  )?.value;
  ...
  if (
    ...
    reShippingType &&
    !BIN_PACKING_TYPES.includes(reShippingType)
  ) {
