        ...
        <div class="group-item p-4 border border-default rounded-lg bg-[hsl(var(--card))]" data-index="${gIdx}">
            ...
            <div class="mb-4">
                 ...
                 <div class="max-h-40 overflow-y-auto border border-default rounded-lg p-2 bg-[hsl(var(--muted))] scrollbar-thin">
                     ${getSellerProducts(session, seller)
                       .map(
                         (prod) => `
            ...
            <div class="step-3-group">
                 ${renderCalculationRules(
                   `group_${safeSellerId}_${gIdx}`,
                   group.calculationMethod || { type: "fixed" },
                   {
...
export function renderGlobalFreeShippingContainer(rule, billingMethod) {
  ...
  const hasThreshold =
    globalThreshold !== null && globalThreshold !== undefined && globalThreshold !== "";
                        ...
                        return `
                                <div class="flex items-center justify-between p-3 bg-[hsl(var(--card))] rounded-lg border border-default forwarder-chain-item" data-index="${idx}" data-forwarder-id="${link.forwarderId}">
                                    ...
                                    <div class="flex space-x-2">
                                        ${
                                          idx > 0
                                            ? `<button class="move-up-btn p-1 secondary-text hover:text-primary transition-colors" data-index="${idx}" title="${t("deliveryRules.moveUp")}">
                                            <span class="icon icon-up h-5 w-5">↑</span>
                                        </button>`
                                            : '<div class="w-7"></div>'
                                        }
                                        ${
                                          idx < forwarderChain.length - 1
                                            ? `<button class="move-down-btn p-1 secondary-text hover:text-primary transition-colors" data-index="${idx}" title="${t("deliveryRules.moveDown")}">
...
export function renderCustomConfigContainer(
  session,
  seller,
  rule,
  safeSellerId,
...
export function renderSellerRulesView({
  session,
  seller,
  rule,
  safeSellerId,
