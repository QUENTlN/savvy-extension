...
function renderCalcMethodDetails(calcMethod, currency, indent = false) {
  ...
  if (type === "free") {
    ...
    const baseLabel =
      base === "order" ? t("deliveryRules.baseOrder") : t("deliveryRules.baseDelivery");
    html = `
      <div class="${indentClass} text-sm secondary-text">
    ...
    if (type === "dimension" && ranges.length > 0) {
      ...
      if (isTiered) {
        ...
        const tierValueMode = ["dimension", "weight_volume", "weight_dimension"].includes(type)
          ? "total"
          : calcMethod.tierValueMode || "total";
        ...
        switch (normalizedType) {
          ...
          `;
            tableRows = ranges
              .map((range) => {
                const min =
                  range.min !== undefined && range.min !== null ? formatNumber(range.min) : "";
                const max =
                  range.max !== undefined && range.max !== null ? formatNumber(range.max) : "∞";
                return `
                <tr class="border-b border-default/50">
        ...
        const showPackingMode = ["weight_volume", "weight_dimension", "volume_packages"].includes(
          normalizedType
        );
        const allowConversion = calcMethod.allowConversion || false;
...
export function renderSellerRecapCard(session, seller, rule) {
  ...
  if (copiedFrom) {
    ...
    if (
      rule.globalFreeShippingThreshold !== null &&
      rule.globalFreeShippingThreshold !== undefined &&
      rule.globalFreeShippingThreshold !== ""
    ) {
      detailsHtml += `
