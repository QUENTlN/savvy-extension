...
export function setupAddGroupButtonListener(session, seller, safeSellerId) {
  ...
  addGroupBtn.addEventListener("click", () => {
    ...
    const newGroupHtml = renderGroupItem(
      session,
      seller,
      { name: t("deliveryRules.newGroupPlaceholder") },
      index,
      safeSellerId,
      getSellerProducts,
      rule
...
export function handleAddForwarderToChain(session, seller) {
  showAssignForwarderModal(session, (forwarderId) => {
    ...
    const maxOrder =
      rule.forwarderChain.length > 0 ? Math.max(...rule.forwarderChain.map((f) => f.order)) : -1;
...
export function saveSellerRules(seller, safeSellerId, session) {
  ...
  if (copiedFromValue) {
    ...
    const billingRadio = document.querySelector(
      `input[name="billing-method-${safeSellerId}"]:checked`
    );
    ...
    if (currentRule.billingMethod === "global") {
      ...
      const globalDistanceInput = document.querySelector(
        `[name="global_${safeSellerId}_distanceValue"]`
      );
      const globalDistanceUnitSelect = document.querySelector(
        `[name="global_${safeSellerId}_unit"]`
      );
      ...
      document.querySelectorAll(".group-item").forEach((groupDiv, idx) => {
        ...
        const productIds = Array.from(
          groupDiv.querySelectorAll(".group-product-checkbox:checked")
        ).map((cb) => cb.value);
        ...
        // Extract distance from calculation method form (only shown when type='distance')
        const groupDistanceInput = groupDiv.querySelector(
          `[name="group_${safeSellerId}_${idx}_distanceValue"]`
        );
        const groupDistanceUnitSelect = groupDiv.querySelector(
          `[name="group_${safeSellerId}_${idx}_unit"]`
        );
