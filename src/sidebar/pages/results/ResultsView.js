...
export function renderResultsView({ result, session, isHistorical, hasHistory }) {
  ...
  const forwarderTotal =
    (totals.forwarder?.reception ?? 0) +
    (totals.forwarder?.storage ?? 0) +
    (totals.forwarder?.repackaging ?? 0) +
    (totals.forwarder?.reshipping ?? 0);
...
function getCustomsBreakdownTooltip(customs, currency) {
  ...
  if (customs.duties?.amount > 0) {
    ...
    parts.push(
      `<div class="flex justify-between gap-4"><span>${t("results.customsCost")}:${rate}</span> <span class="font-medium">${formatCurrency(customs.duties.amount, currency)}</span></div>`
    );
  }
  if (customs.vat?.amount > 0) {
    const rate = customs.vat.rate ? ` (${formatPercent(customs.vat.rate)}%)` : "";
    parts.push(
      `<div class="flex justify-between gap-4"><span>${t("results.vatCost")}:${rate}</span> <span class="font-medium">${formatCurrency(customs.vat.amount, currency)}</span></div>`
    );
  }
  if (customs.clearance?.amount > 0)
    parts.push(
      `<div class="flex justify-between gap-4"><span>${t("results.customsClearanceFee")}:</span> <span class="font-medium">${formatCurrency(customs.clearance.amount, currency)}</span></div>`
    );
  return parts.join("");
}
...
function getForwarderBreakdownTooltip(forwarder, currency) {
  if (!forwarder) return "";
  const parts = [];
  if (forwarder.reception?.amount > 0)
    parts.push(
      `<div class="flex justify-between gap-4"><span>${t("results.forwarderReception")}:</span> <span class="font-medium">${formatCurrency(forwarder.reception.amount, currency)}</span></div>`
    );
  if (forwarder.storage?.amount > 0)
    parts.push(
      `<div class="flex justify-between gap-4"><span>${t("results.forwarderStorage")}:</span> <span class="font-medium">${formatCurrency(forwarder.storage.amount, currency)}</span></div>`
    );
  if (forwarder.repackaging?.amount > 0)
    parts.push(
      `<div class="flex justify-between gap-4"><span>${t("results.forwarderRepackaging")}:</span> <span class="font-medium">${formatCurrency(forwarder.repackaging.amount, currency)}</span></div>`
    );
  if (forwarder.reshipping?.amount > 0)
    parts.push(
      `<div class="flex justify-between gap-4"><span>${t("results.forwarderReShipping")}:</span> <span class="font-medium">${formatCurrency(forwarder.reshipping.amount, currency)}</span></div>`
    );
  return parts.join("");
}
...
function renderSellerCard(seller, sellerData, session, currency, groups) {
  ...
  const processItem = (item) => {
    ...
    if (item.costs?.insurance?.amount) {
      ...
      if (item.ids?.offer) {
        ...
        if (originalOffer?.insurancePrice)
          itemInsurance = originalOffer.insurancePrice * item.quantity;
      } else if (item.ids?.bundle) {
        // Bundle
        const bundleId = item.ids.bundle;
        const originalBundle = session.bundles?.find((b) => b.id === bundleId);
        if (originalBundle?.insurancePrice)
          itemInsurance = originalBundle.insurancePrice * item.quantity;
      }
    }
  ...
  const sellerTotal =
    productsTotal + shippingAmount + insuranceTotal + customsTotal + forwarderTotal;
...
function renderOfferRow(offer, session, currency, columns) {
  ...
  if (offer.costs?.customs) {
    customs +=
      (offer.costs.customs.duties?.amount || 0) +
      (offer.costs.customs.vat?.amount || 0) +
      (offer.costs.customs.clearance?.amount || 0);
  }
  ...
  if (offer.costs?.forwarder) {
    forwarder +=
      (offer.costs.forwarder.reception?.amount || 0) + (offer.costs.forwarder.storage?.amount || 0);
    forwarder +=
      (offer.costs.forwarder.repackaging?.amount || 0) +
      (offer.costs.forwarder.reshipping?.amount || 0);
  }
...
function renderBundleRow(bundle, session, currency, columns) {
  ...
  if (bundle.costs?.customs) {
    customs +=
      (bundle.costs.customs.duties?.amount || 0) +
      (bundle.costs.customs.vat?.amount || 0) +
      (bundle.costs.customs.clearance?.amount || 0);
  }
  ...
  if (bundle.costs?.forwarder) {
    forwarder +=
      (bundle.costs.forwarder.reception?.amount || 0) +
      (bundle.costs.forwarder.storage?.amount || 0) +
      (bundle.costs.forwarder.repackaging?.amount || 0) +
      (bundle.costs.forwarder.reshipping?.amount || 0);
  }
