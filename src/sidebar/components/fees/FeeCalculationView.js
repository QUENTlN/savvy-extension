 ...
 */
export function renderSimplePackingModeSelector(
  prefix,
  packingMode = "single",
  context = "default"
) {
...
export function renderRangeRow(
  type,
  prefix,
  idx,
  range = {},
  tierValueType = "fixed",
  tierValueMode = "perUnit",
  unit = "",
  unit2 = "",
  currency = null
) {
  ...
  const displayValue =
    range.value !== undefined && range.value !== ""
      ? isPercentType
...
export function renderTieredInputs(
  prefix,
  data,
  type,
  hideAdvancedSettings = false,
  currency = null,
  showPackingMode = false,
  distanceConfig = null
) {
  const currencyCode = currency || DEFAULT_CURRENCY;
...
export function renderOrderAmountInputs(
  prefix,
  data,
  hideAdvancedSettings = false,
  currency = null,
  ...
  const valueLabel =
    tierValueType === "fixed"
      ? currencySymbol
      : tierValueType === "pctOrder"
        ? "%commande"
        : tierValueType === "pctDelivery"
          ? "%livraison"
          : currencySymbol;
...
export function renderCombinedInputs(prefix, data, type, currency = null, canConvert = false) {
  ...
  if (canConvert) {
    const allowConversion = data.allowConversion || false;
    const conversionHelpKey =
      type === "weight_dimension" ? "allowConversionDimensionHelp" : "allowConversionVolumeHelp";
    html += `
            <div class="mb-4">
                <label class="flex items-center gap-2 cursor-pointer group">
                    <input type="checkbox" name="${prefix}_allowConversion"
...
export function renderCalculationRules(prefix, ruleData, config = {}) {
  ...
  const typeDefinitions = {
    ...
    free: {
      label:
        labelContext === "forwarder"
          ? t("forwarderFees.typeFree")
          : t("deliveryRules.freeDelivery"),
      help:
        labelContext === "forwarder"
          ? t("forwarderFees.typeFreeHelp")
          : t("deliveryRules.freeDeliveryHelp"),
    },
    fixed: {
      label:
        labelContext === "forwarder" ? t("forwarderFees.typeFixed") : t("deliveryRules.typeFixed"),
      help:
        labelContext === "forwarder"
          ? t("forwarderFees.typeFixedHelp")
          : t("deliveryRules.typeFixedHelp"),
    },
    percentage: {
      label:
        labelContext === "forwarder"
          ? t("forwarderFees.typePercentage")
          : t("deliveryRules.typePercentage"),
      help:
        labelContext === "forwarder"
          ? t("forwarderFees.typePercentageHelp")
          : t("deliveryRules.typePercentageHelp"),
    },
    ...
    order_amount: {
      label: t("deliveryRules.typeOrderAmount"),
      help: t("deliveryRules.typeOrderAmountHelp"),
    },
    dimension: {
      label: t("deliveryRules.typeDimension"),
      help: t("deliveryRules.typeDimensionHelp"),
    },
    weight_volume: {
      label: t("deliveryRules.typeWeightVolume"),
      help: t("deliveryRules.typeWeightVolumeHelp"),
    },
    weight_dimension: {
      label: t("deliveryRules.typeWeightDimension"),
      help: t("deliveryRules.typeWeightDimensionHelp"),
    },
    volume_packages: {
      label: t("deliveryRules.typeVolumePackages"),
      help: t("deliveryRules.typeVolumePackagesHelp"),
    },
  };
  ...
  const freeLabel =
    labelContext === "forwarder" ? t("forwarderFees.typeFree") : t("deliveryRules.freeDelivery");
  ...
  const TYPE_RENDERERS = {
    cumul: (p, d) =>
      `<p class="text-sm secondary-text italic mb-3">${t("deliveryRules.typeCumul")}</p>` +
      (showPackingMode ? renderSimplePackingModeSelector(p, d.packingMode || "single") : ""),
    free: (p, d) =>
      `<p class="text-sm font-medium card-text italic mb-3">${freeLabel}</p>` +
      (showPackingMode ? renderSimplePackingModeSelector(p, d.packingMode || "single") : ""),
    fixed: (p, d) => renderFixedInputs(p, d, currencyCode, showPackingMode),
    percentage: (p, d) => renderPercentageInputs(p, d, showPackingMode),
    quantity: (p, d) =>
      renderTieredInputs(p, d, "quantity", hideAdvancedSettings, currencyCode, showPackingMode),
    distance: (p, d) =>
      renderTieredInputs(
        p,
        d,
    ...
    weight: (p, d) =>
      renderTieredInputs(p, d, "weight", hideAdvancedSettings, currencyCode, showPackingMode),
    volume: (p, d) =>
      renderTieredInputs(p, d, "volume", hideAdvancedSettings, currencyCode, showPackingMode),
    order_amount: (p, d) =>
      renderOrderAmountInputs(p, d, hideAdvancedSettings, currencyCode, showPackingMode),
    ...
    weight_dimension: (p, d) =>
      renderCombinedInputs(p, d, "weight_dimension", currencyCode, canConvert),
