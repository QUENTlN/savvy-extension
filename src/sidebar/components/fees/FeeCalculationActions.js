...
export async function handleCalculationTypeChange(e) {
  ...
  const isSellerShipping =
    prefix.includes("seller") || prefix.includes("global") || prefix.includes("group");
  ...
  if (newType === "fixed") {
    ...
    const data = {
      type: newType,
      weightUnit: getDefaultUnitForType("weight"),
      volUnit: getDefaultUnitForType("volume"),
    };
...
export function handleTieredToggle(e) {
  ...
  data.isTiered = element.classList.contains("is-tiered-toggle")
    ? element.value === "tiered"
    : element.checked;
  ...
  if (["weight_volume", "weight_dimension"].includes(type)) {
    ...
    inputsContainer.innerHTML = renderTieredInputs(
      prefix,
      data,
      type,
      false,
...
export function handleMaxChange(e, container) {
  ...
  if (nextRow && nextRow.classList.contains("range-row")) {
    ...
    if (
      !["dimension", "weight_volume", "weight_dimension"].includes(type) &&
      !input.name.includes("_maxL") &&
      !input.name.includes("_maxW") &&
      !input.name.includes("_maxH") &&
      !input.name.includes("_maxWeight") &&
      !input.name.includes("_maxVol")
    ) {
